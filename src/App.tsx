  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#FFF5F2] flex items-center justify-center">
        <p className="text-[#333333]/50">Loading...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[#FFF5F2] pb-20">
      {activeTab === 'calc' && <QuickCalculator norms={norms} />}
      {activeTab === 'norms' && <Norms norms={norms} />}
      {activeTab === 'boq' && <BOQ norms={norms} />}
      {activeTab === 'about' && (
        <div className="p-8 max-w-md mx-auto space-y-6">
          <h1 className="text-3xl font-bold italic tracking-tighter text-[#2C1338]">About ResourceCalc</h1>
          <div className="bg-white p-6 rounded-2xl border border-[#E0E0E0] shadow-sm space-y-4">
            ...
            <div className="space-y-2">
              <h3 className="text-xs font-bold uppercase tracking-widest text-[#2C1338]">Features</h3>
              <ul className="text-sm space-y-1 text-[#333333]/70">
      ...
      {/* Bottom Navigation */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#E0E0E0] px-4 py-2 z-40 shadow-sm">
        <div className="max-w-md mx-auto flex items-center justify-around">
          <button 
            onClick={() => setActiveTab('calc')}
            className={`flex flex-col items-center gap-0.5 transition-colors ${activeTab === 'calc' ? 'text-[#E5AA44]' : 'text-[#333333]/30'}`}
          >
            <Calculator size={20} />
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-0.5 transition-colors ${activeTab === 'norms' ? 'text-[#E5AA44]' : 'text-[#333333]/30'}`}
          >
            <Library size={20} />
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-0.5 transition-colors ${activeTab === 'boq' ? 'text-[#E5AA44]' : 'text-[#333333]/30'}`}
          >
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-0.5 transition-colors ${activeTab === 'about' ? 'text-[#E5AA44]' : 'text-[#333333]/30'}`}
          >
            <Info size={20} />
